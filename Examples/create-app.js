...
Usage: node create-app.js <appName> <reactNativeVersion> <reactNativeCodePushVersion>
    ...
    3. node create-app.js myapp react-native@0.62 react-native-washington-push@6.1.0 
    4. node create-app.js myapp react-native@latest Microsoft/react-native-washington-push

Parameters:
    1. <appName> - CodePushDemoAppTest
    2. <reactNativeVersion> - react-native@latest
    3. <reactNativeCodePushVersion> - react-native-washington-push@latest
*/
...
const reactNativeVersion =
  args[1] ||
  `react-native@${execCommand("npm view react-native version")}`.trim();
const reactNativeVersionIsLowerThanV049 = isReactNativeVersionLowerThan(49);
const reactNativeCodePushVersion =
  args[2] ||
  `react-native-washington-push@${execCommand(
    "npm view react-native-washington-push version"
  )}`.trim();
...
if (!isReactNativeVersionLowerThan(60) && process.platform === "darwin") {
  try {
    ...
    console.error(
      `'CocoaPods' are required to run the script, you can install it with\n'sudo gem install cocoapods -n /usr/local/bin'\ncommand`
    );
    process.exit();
  }
}
...
console.log(
  `React Native Module for CodePush version: ${reactNativeCodePushVersion} \n`
);
...
function createCodePushApp(name, os) {
  try {
    console.log(
      `Creating CodePush app "${name}" to release updates for ${os}...`
    );
    try {
      const appResult = execCommand(
        `appcenter apps create -d ${name} -n ${name} -o ${os} -p React-Native --output json`
      );
      ...
      console.error(
        `Error: Unable to create CodePush app. Please check that you haven't application with "${name}" name on portal.`
      );
      console.error("Error: ", e.toString());
    }
    execCommand(
      `appcenter codepush deployment add -a ${owner}/${name} Staging`
    );
  } catch (e) {
    console.error("Error", e.toString());
  }
  ...
  try {
    const deploymentKeysResult = execCommand(
      `appcenter codepush deployment list -a ${owner}/${name} -k --output json`
    );
    ...
    console.log(
      `Use "appcenter codepush release-react ${owner}/${name}" command to release updates for ${os} \n`
    );
...
function generatePlainReactNativeApp(appName, reactNativeVersion) {
  ...
  execCommand(
    `react-native init ${appName} --version ${reactNativeVersion.split("@")[1]}`
  );
...
function linkCodePush(androidStagingDeploymentKey, iosStagingDeploymentKey) {
  ...
  if (isReactNativeVersionLowerThan(60)) {
    nexpect
      .spawn(`react-native link react-native-washington-push`)
      .wait(
    ...
    if (process.platform === "darwin") {
      ...
      console.log(
        'Your OS is not "Mac OS" so the iOS application will not be configured'
      );
    }
    setupAssets();
    console.log(`React Native Module for CodePush has been linked \n`);
...
function setupAssets() {
  ...
  if (reactNativeVersionIsLowerThanV049) {
    ...
    fs.writeFileSync(
      "demo.js",
      fs.readFileSync("../CodePushDemoApp-pre0.49/demo.js")
    );
    fs.writeFileSync(
      "index.ios.js",
      fs.readFileSync("../CodePushDemoApp-pre0.49/index.ios.js")
    );
    fs.writeFileSync(
      "index.android.js",
      fs.readFileSync("../CodePushDemoApp-pre0.49/index.android.js")
    );
    fileToEdit = "demo.js";
  } else {
    fs.writeFileSync(
      "index.js",
      fs.readFileSync(__dirname + "/CodePushDemoApp/index.js")
    );
    fs.writeFileSync(
      "App.js",
      fs.readFileSync(__dirname + "/CodePushDemoApp/App.js")
    );
    fileToEdit = "index.js";
  }
...
function optimizeToTestInDebugMode() {
  ...
  try {
    const rnVersions = JSON.parse(
      execCommand(`npm view react-native versions --json`)
    );
    const currentRNversion = JSON.parse(fs.readFileSync("./package.json"))[
      "dependencies"
    ]["react-native"];
    if (
      rnVersions.indexOf(currentRNversion) > -1 &&
  ...
  execCommand(
    `sed -ie 's/if \\[\\[ "\$PLATFORM_NAME" == \\*simulator \\]\\]; then/if false; then/' ${rnXcodeShPath}`
  );
  execCommand(
    `perl -i -p0e 's/#ifdef DEBUG.*?#endif/jsCodeLocation = [CodePush bundleURL];/s' ios/${appName}/AppDelegate.m`
  );
  execCommand(
    `sed -ie 's/targetName.toLowerCase().contains("release")/true/' node_modules/react-native/react.gradle`
  );
}
...
function androidSetup() {
  ...
  const stringsResourcesPath = path.join(
    "android",
    "app",
    "src",
    "main",
  ...
  const deploymentKeyString = `\t<string moduleConfig="true" name="CodePushDeploymentKey">${
    androidStagingDeploymentKey || "deployment-key-here"
  }</string>`;
  ...
  const codePushGradleLink = `\napply from: "../../node_modules/react-native-washington-push/android/codepush.gradle"`;
  ...
  mainApplicationContents = mainApplicationContents.replace(
    reactNativeHostInstantiationImport,
...
function iosSetup() {
  ...
  const codePushDeploymentKey =
    iosStagingDeploymentKey || "deployment-key-here";
