...
module.exports = {
  checkForUpdate: function (testApp, onSuccess, onError, deploymentKey) {
    return CodePush.checkForUpdate(deploymentKey).then(
      (remotePackage) => {
        return testApp.checkUpdateSuccess(remotePackage).then(() => {
          return onSuccess && onSuccess(remotePackage);
        });
      },
      (error) => {
        return testApp.checkUpdateError(error).then(() => {
  ...
  download: function (testApp, onSuccess, onError, remotePackage) {
    return remotePackage.download().then(
      (localPackage) => {
        return testApp.downloadSuccess(localPackage).then(() => {
          return onSuccess && onSuccess(localPackage);
        });
      },
      (error) => {
        return testApp.downloadError(error).then(() => {
          return onError && onError(error);
        });
      }
    );
  ...
  install: function (
    ...
    return localPackage.install(installMode, minBackgroundDuration).then(
      () => {
        ...
        if (installMode !== CodePush.InstallMode.IMMEDIATE || disallowRestart)
          return testApp.installSuccess().then(() => {
            return onSuccess && onSuccess();
          });
        return onSuccess && onSuccess();
      },
      () => {
        return testApp.installError().then(() => {
  ...
  checkAndInstall: function (
    ...
    var installUpdate = this.install.bind(
      this,
      testApp,
      onSuccess,
      onError,
    ...
    var downloadUpdate = this.download.bind(
      this,
      testApp,
      installUpdate,
      onError
  ...
  sync: function (testApp, onSyncStatus, onSyncError, options) {
    return CodePush.checkForUpdate().then(
      (remotePackage) => {
        ...
        var regularUpdateIsImmediate =
          options && options.installMode === CodePush.InstallMode.IMMEDIATE;
        var mandatoryUpdateIsImmediate =
          !options ||
          (options &&
            (!options.mandatoryInstallMode ||
              options.mandatoryInstallMode === CodePush.InstallMode.IMMEDIATE));
        var isInstallImmediate =
          remotePackage && remotePackage.isMandatory
            ? mandatoryUpdateIsImmediate
            : regularUpdateIsImmediate;

        return CodePush.sync(options).then(
          (status) => {
            if (
              !(
                isInstallImmediate &&
              ...
              return testApp.onSyncStatus(status).then(() => {
          ...
          (error) => {
            return testApp.onSyncError(error).then(() => {
