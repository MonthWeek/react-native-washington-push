...
class CODE_PUSH_TEST_APP_NAME extends Component {
    ...
    if (remotePackage) {
      if (!remotePackage.failedInstall) {
        return this.setStateAndSendMessage(
          "There is an update available. Remote package:" +
        ...
        return this.setStateAndSendMessage(
          "An update is available but failed previously. Remote package:" +
            JSON.stringify(remotePackage),
  ...
  sendCurrentAndPendingPackage() {
    return CodePush.getUpdateMetadata(CodePush.UpdateState.PENDING)
      .then((pendingPackage) => {
        this.setStateAndSendMessage(
          "Pending package: " + pendingPackage,
          "PENDING_PACKAGE",
          [pendingPackage ? pendingPackage.packageHash : null]
        );
      ...
      .then((currentPackage) => {
        return this.setStateAndSendMessage(
          ...
          "CURRENT_PACKAGE",
          [currentPackage ? currentPackage.packageHash : null]
        );
      });
  }
  ...
  render() {
    return (
      <View style={styles.container}>
        <Text style={styles.welcome}>CodePush React-Native Plugin Tests</Text>
