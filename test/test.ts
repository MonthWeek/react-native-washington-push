...
class RNAndroid extends Platform.Android implements RNPlatform {
   ...
   */
  installPlatform(projectDirectory: string): Q.Promise<void> {
    const innerprojectDirectory: string = path.join(
      projectDirectory,
      TestConfig.TestAppName
    );
    ...
    TestUtil.replaceString(
      settingsGradle,
      "include ':app'",
      "include ':app', ':react-native-washington-push'\nproject(':react-native-washington-push').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-washington-push/android/app')"
    );
    ...
    TestUtil.replaceString(
      string,
      TestUtil.ANDROID_KEY_PLACEHOLDER,
   ...
   */
  installApp(projectDirectory: string): Q.Promise<void> {
    const androidDirectory: string = path.join(
      projectDirectory,
      TestConfig.TestAppName,
      "android"
    );
    return TestUtil.getProcessOutput(
      "adb install -r " + this.getBinaryPath(projectDirectory),
  ...
  buildFunction(androidDirectory: string): Q.Promise<void> {
    if (process.platform === "darwin") {
      return TestUtil.getProcessOutput(`./gradlew assembleRelease --daemon`, {
        noLogStdOut: true,
        cwd: androidDirectory,
      }).then(() => {
        return null;
...
class RNIOS extends Platform.IOS implements RNPlatform {
   ...
   */
  installPlatform(projectDirectory: string): Q.Promise<void> {
    const iOSProject: string = path.join(
      projectDirectory,
      TestConfig.TestAppName,
      "ios"
    );
    const infoPlistPath: string = path.join(
      iOSProject,
      TestConfig.TestAppName,
      "Info.plist"
    );
    const appDelegatePath: string = path.join(
      iOSProject,
      TestConfig.TestAppName,
      "AppDelegate.m"
    );
    ...
    // Install the Podfile
    return (
      TestUtil.getProcessOutput("pod install", { cwd: iOSProject })
        ...
        .then(
          TestUtil.replaceString.bind(undefined, infoPlistPath, "1.0", "1.0.0")
        )
        ...
        .then(
          TestUtil.replaceString.bind(
            ...
            path.join(
              iOSProject,
              TestConfig.TestAppName + ".xcodeproj",
              "project.pbxproj"
            ),
            '"[$][(]inherited[)]",\\s*[)];',
            '"$(inherited)"\n\t\t\t\t);'
  ...
  installApp(projectDirectory: string): Q.Promise<void> {
    return TestUtil.getProcessOutput(
      "xcrun simctl install booted " + this.getBinaryPath(projectDirectory)
    ).then(() => {
      return null;
    });
  }
      ...
      .then((targetEmulator: string) => {
        ...
        return TestUtil.getProcessOutput(
          "xcodebuild -workspace " +
            path.join(iOSProject, TestConfig.TestAppName) +
            ".xcworkspace -scheme " +
            TestConfig.TestAppName +
            ' -configuration Release -destination "platform=iOS Simulator,id=' +
            hash +
            '" -derivedDataPath build EXCLUDED_ARCHS=arm64',
          { cwd: iOSProject, maxBuffer: 1024 * 1024 * 500, noLogStdOut: true }
        );
      })
      .then<void>(
        () => {
...
const supportedTargetPlatforms: Platform.IPlatform[] = [
  new RNAndroid(),
  new RNIOS(),
];
...
class RNProjectManager extends ProjectManager {
  ...
  public copyTemplate(
    ...
    return copyDirectoryRecursively(
      templatePath,
      path.join(projectDirectory, TestConfig.TestAppName)
    );
  }
  ...
  public setupProject(
    ...
    return TestUtil.getProcessOutput("react-native init " + appName, {
      cwd: projectDirectory,
      timeout: 30 * 60 * 1000,
    })
      .then((e) => {
        console.log(
          `"react-native init ${appName}" success. cwd=${projectDirectory}`
        );
        return e;
      })
      ...
      .then<void>(
        TestUtil.getProcessOutput.bind(
          undefined,
          TestConfig.thisPluginInstallString,
      ...
      .catch((error) => {
        console.log(
          `"react-native init ${appName} failed". cwd=${projectDirectory}`,
          error
        );
  ...
  public createUpdateArchive(
    ...
    const bundleFolder: string = path.join(
      projectDirectory,
      TestConfig.TestAppName,
      "CodePush/"
    );
    const bundleName: string = (<RNPlatform>(
      (<any>targetPlatform)
    )).getBundleName();
   ...
   */
  public preparePlatform(
    projectDirectory: string,
    targetPlatform: Platform.IPlatform
  ): Q.Promise<void> {
    const deferred = Q.defer<string>();

    const platformsJSONPath = path.join(
      projectDirectory,
      RNProjectManager.platformsJSON
    );
    ...
    fs.exists(platformsJSONPath, (exists) => {
      ...
      const platformJSON = eval(
        "(" + fs.readFileSync(platformsJSONPath, "utf8") + ")"
      );
    ...
    return deferred.promise.then<void>(
      () => {
        return (<RNPlatform>(<any>targetPlatform)).installPlatform(
          projectDirectory
        );
      },
      (error: any) => {
  ...
  public runApplication(
    ...
    return Q<void>(null)
      ...
      .then(() => {
        ...
        return (<RNPlatform>(<any>targetPlatform))
          .installApp(projectDirectory)
          .then<void>(
            targetPlatform
              .getEmulatorManager()
              .launchInstalledApplication.bind(
                undefined,
...
const ScenarioInstallOnResumeWithRevert =
  "scenarioInstallOnResumeWithRevert.js";
const ScenarioInstallOnSuspendWithRevert =
  "scenarioInstallOnSuspendWithRevert.js";
const ScenarioInstallOnRestartWithRevert =
...
PluginTestingFramework.initializeTests(
  ...
  (projectManager: ProjectManager, targetPlatform: Platform.IPlatform) => {
    TestBuilder.describe(
      ...
      () => {
        TestBuilder.it(
          "window.codePush.checkForUpdate.noUpdate",
          false,
          (done: Mocha.Done) => {
            ...
            ServerUtil.testMessageCallback = (requestBody: any) => {
              try {
                assert.strictEqual(
                  requestBody.message,
                  ServerUtil.TestMessage.CHECK_UP_TO_DATE
                );
                done();
              } catch (e) {
                done(e);
        ...
        TestBuilder.it(
          "window.codePush.checkForUpdate.sendsBinaryHash",
          false,
          (done: Mocha.Done) => {
            if (!(<RNPlatform>(<any>targetPlatform)).isDiffsSupported()) {
              console.log(
                targetPlatform.getName() + " does not send a binary hash!"
              );
              done();
              return;
            }
        ...
        TestBuilder.it(
          "window.codePush.checkForUpdate.noUpdate.updateAppVersion",
          false,
          (done: Mocha.Done) => {
            ...
            ServerUtil.updateResponse = {
              update_info: updateAppVersionResponse,
            };
            ...
            ServerUtil.testMessageCallback = (requestBody: any) => {
              try {
                ...
                assert.strictEqual(
                  ...
                  updateResponse.is_mandatory
                );
                assert.strictEqual(remotePackage.label, updateResponse.label);
                assert.strictEqual(
                  remotePackage.packageHash,
                  updateResponse.package_hash
                );
                assert.strictEqual(
                  remotePackage.packageSize,
                  updateResponse.package_size
                );
                assert.strictEqual(
                  remotePackage.deploymentKey,
                  targetPlatform.getDefaultDeploymentKey()
                );
                done();
              } catch (e) {
                done(e);
        ...
        TestBuilder.it(
          "window.codePush.checkForUpdate.error",
          false,
          (done: Mocha.Done) => {
            ...
            ServerUtil.testMessageCallback = (requestBody: any) => {
              try {
                assert.strictEqual(
                  requestBody.message,
                  ServerUtil.TestMessage.CHECK_ERROR
                );
                done();
              } catch (e) {
                done(e);
    ...
    TestBuilder.describe(
      ...
      () => {
        TestBuilder.it(
          "window.codePush.checkForUpdate.customKey.update",
          false,
          (done: Mocha.Done) => {
            const updateResponse = ServerUtil.createUpdateResponse();
            ServerUtil.updateResponse = { update_info: updateResponse };
    ...
    TestBuilder.describe(
      ...
      () => {
        TestBuilder.it(
          "remotePackage.download.success",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
                false,
                targetPlatform
              ),
            };
            ...
            ServerUtil.updatePackagePath = path.join(
              TestConfig.templatePath,
              "index.js"
            );
        ...
        TestBuilder.it(
          ...
          (done: Mocha.Done) => {
            ...
            ServerUtil.updateResponse.update_info.download_url =
              "http://invalid_url";
    ...
    TestBuilder.describe(
      ...
      () => {
        ...
        TestBuilder.it(
          "localPackage.install.handlesDiff.againstBinary",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
        ...
        TestBuilder.it(
          "localPackage.install.immediately",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
    ...
    TestBuilder.describe(
      ...
      () => {
        TestBuilder.it(
          "localPackage.install.revert.dorevert",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
        ...
        TestBuilder.it(
          "localPackage.install.revert.norevert",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
    ...
    TestBuilder.describe(
      ...
      () => {
        TestBuilder.it(
          "localPackage.installOnNextResume.dorevert",
          true,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
            ...
            setupUpdateScenario(
              ...
              .then<void>(() => {
                ...
                targetPlatform
                  .getEmulatorManager()
                  .resumeApplication(TestConfig.TestNamespace);
                return ServerUtil.expectTestMessages([
                  ServerUtil.TestMessage.DEVICE_READY_AFTER_UPDATE,
                ]);
              })
        ...
        TestBuilder.it(
          "localPackage.installOnNextResume.norevert",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
    ...
    TestBuilder.describe(
      ...
      () => {
        TestBuilder.it(
          "localPackage.installOnNextSuspend.dorevert",
          true,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
        ...
        TestBuilder.it(
          "localPackage.installOnNextSuspend.norevert",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
    ...
    TestBuilder.describe(
      ...
      () => {
        TestBuilder.it(
          "localPackage.installOnNextRestart.dorevert",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
        ...
        TestBuilder.it(
          "localPackage.installOnNextRestart.norevert",
          true,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
        ...
        TestBuilder.it(
          "localPackage.installOnNextRestart.revertToPrevious",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
            ...
            setupUpdateScenario(
              ...
              .then<void>(() => {
                ...
                ServerUtil.testMessageResponse =
                  ServerUtil.TestMessageResponse.SKIP_NOTIFY_APPLICATION_READY;
                targetPlatform
                  .getEmulatorManager()
                  .restartApplication(TestConfig.TestNamespace);
                return ServerUtil.expectTestMessages([
    ...
    TestBuilder.describe(
      ...
      () => {
        TestBuilder.it(
          "codePush.restartApplication.checkPackages",
          true,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
            ...
            setupUpdateScenario(
              ...
              .then<void>((updatePath: string) => {
                ...
                return ServerUtil.expectTestMessages([
                  new ServerUtil.AppMessage(
                    ServerUtil.TestMessage.PENDING_PACKAGE,
                    [null]
                  ),
                  new ServerUtil.AppMessage(
                    ServerUtil.TestMessage.CURRENT_PACKAGE,
                    [null]
                  ),
                  new ServerUtil.AppMessage(
                    ServerUtil.TestMessage.SYNC_STATUS,
    ...
    TestBuilder.describe("#codePush.restartApplication.2x", () => {
      TestBuilder.it(
        ...
        (done: Mocha.Done) => {
          ServerUtil.updateResponse = {
            update_info: ServerUtil.createUpdateResponse(false, targetPlatform),
          };
          setupTestRunScenario(
            projectManager,
            targetPlatform,
      ...
      TestBuilder.it(
        "doesn't block when the restart is ignored",
        false,
        (done: Mocha.Done) => {
          ServerUtil.updateResponse = {
            update_info: ServerUtil.createUpdateResponse(false, targetPlatform),
    ...
    TestBuilder.describe("#window.codePush.sync", () => {
      ...
      TestBuilder.describe(
        ...
        () => {
          ...
          TestBuilder.it(
            "window.codePush.sync.checkerror",
            false,
            (done: Mocha.Done) => {
              ServerUtil.updateResponse = "invalid {{ json";
              ...
              Q({})
                .then<void>((p) => {
                  ...
                  return ServerUtil.expectTestMessages([
                    new ServerUtil.AppMessage(
                      ServerUtil.TestMessage.SYNC_STATUS,
                      [ServerUtil.TestMessage.SYNC_ERROR]
                    ),
                  ]);
                })
                .done(
          ...
          TestBuilder.it(
            "window.codePush.sync.downloaderror",
            false,
            (done: Mocha.Done) => {
              const invalidUrlResponse = ServerUtil.createUpdateResponse();
              invalidUrlResponse.download_url =
                "http://" +
                path.join(TestConfig.templatePath, "invalid_path.zip");
      ...
      TestBuilder.describe(
        ...
        () => {
          ...
          TestBuilder.it(
            "window.codePush.sync.2x.noupdate",
            false,
            (done: Mocha.Done) => {
              const noUpdateResponse = ServerUtil.createDefaultResponse();
              noUpdateResponse.is_available = false;
          ...
          TestBuilder.it(
            "window.codePush.sync.2x.checkerror",
            false,
            (done: Mocha.Done) => {
              ServerUtil.updateResponse = "invalid {{ json";
          ...
          TestBuilder.it(
            "window.codePush.sync.2x.downloaderror",
            false,
            (done: Mocha.Done) => {
              const invalidUrlResponse = ServerUtil.createUpdateResponse();
              invalidUrlResponse.download_url =
          ...
          TestBuilder.it(
            "window.codePush.sync.2x.dorevert",
            false,
            (done: Mocha.Done) => {
              ServerUtil.updateResponse = {
                update_info: ServerUtil.createUpdateResponse(
          ...
          TestBuilder.it(
            "window.codePush.sync.2x.update",
            true,
            (done: Mocha.Done) => {
              ServerUtil.updateResponse = {
                update_info: ServerUtil.createUpdateResponse(
    ...
    TestBuilder.describe(
      "#window.codePush.sync minimum background duration tests",
      () => {
        TestBuilder.it(
          "defaults to no minimum for Resume mode",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
        ...
        TestBuilder.it(
          "min background duration 5s for Resume mode",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
            ...
            setupTestRunScenario(
              ...
              .then(() => {
                ...
                return targetPlatform
                  .getEmulatorManager()
                  .resumeApplication(TestConfig.TestNamespace, 3 * 1000);
              })
              .then(() => {
                targetPlatform
                  .getEmulatorManager()
                  .resumeApplication(TestConfig.TestNamespace, 6 * 1000);
                return ServerUtil.expectTestMessages([
                  ServerUtil.TestMessage.DEVICE_READY_AFTER_UPDATE,
                  new ServerUtil.AppMessage(
                    ServerUtil.TestMessage.SYNC_STATUS,
        ...
        TestBuilder.it(
          "defaults to no minimum for Suspend mode",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
        ...
        TestBuilder.it(
          "min background duration 5s for Suspend mode",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
    ...
    TestBuilder.describe(
      "#window.codePush.sync mandatory install mode tests",
      () => {
        TestBuilder.it("defaults to IMMEDIATE", false, (done: Mocha.Done) => {
          ServerUtil.updateResponse = {
            update_info: ServerUtil.createUpdateResponse(true, targetPlatform),
          };

          setupTestRunScenario(
            projectManager,
            targetPlatform,
            ScenarioSyncMandatoryDefault
          )
            .then<string>(() => {
              return setupUpdateScenario(
                projectManager,
          ...
          (done: Mocha.Done) => {
            ...
            setupTestRunScenario(
              ...
              .then<void>(() => {
                ...
                targetPlatform
                  .getEmulatorManager()
                  .resumeApplication(TestConfig.TestNamespace, 5 * 1000);
                return ServerUtil.expectTestMessages([
                  ServerUtil.TestMessage.DEVICE_READY_AFTER_UPDATE,
                ]);
              })
        ...
        TestBuilder.it(
          "has no effect on updates that are not mandatory",
          false,
          (done: Mocha.Done) => {
            ServerUtil.updateResponse = {
              update_info: ServerUtil.createUpdateResponse(
    ...
    TestBuilder.describe("#codePush.disallowRestart", () => {
      TestBuilder.it(
        "disallowRestart with IMMEDIATE install mode",
        false,
        (done: Mocha.Done) => {
          ServerUtil.updateResponse = {
            update_info: ServerUtil.createUpdateResponse(false, targetPlatform),
          ...
          setupTestRunScenario(
            projectManager,
            targetPlatform,
            ScenarioDisallowRestartImmediate
          )
            .then(
              setupUpdateScenario.bind(
                this,
      ...
      TestBuilder.it(
        "disallowRestart with ON_NEXT_RESUME install mode",
        false,
        (done: Mocha.Done) => {
          ServerUtil.updateResponse = {
            update_info: ServerUtil.createUpdateResponse(false, targetPlatform),
          ...
          setupTestRunScenario(
            projectManager,
            targetPlatform,
            ScenarioDisallowRestartOnResume
          )
            .then(
              setupUpdateScenario.bind(
                this,
      ...
      TestBuilder.it(
        "disallowRestart with ON_NEXT_SUSPEND install mode",
        false,
        (done: Mocha.Done) => {
          ServerUtil.updateResponse = {
            update_info: ServerUtil.createUpdateResponse(false, targetPlatform),
          ...
          setupTestRunScenario(
            projectManager,
            targetPlatform,
            ScenarioDisallowRestartOnSuspend
          )
            .then(
              setupUpdateScenario.bind(
                this,
