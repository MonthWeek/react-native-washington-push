...
module.exports = () => {
  ...
  if (!appDelegatePath) {
    ...
    https://github.com/microsoft/react-native-washington-push#plugin-configuration-ios`);
  } else {
    var appDelegateContents = fs.readFileSync(appDelegatePath, "utf8");

    // 1. Remove the header import statement
    if (
      !~appDelegateContents.indexOf(linkTools.codePushHeaderImportStatement)
    ) {
    ...
    if (!~appDelegateContents.indexOf(codePushBundleUrl)) {
      console.log(
        `"jsCodeLocation" already not pointing to "[CodePush bundleURL]".`
      );
    } else {
      var reactNativeVersion =
        packageFile &&
        packageFile.dependencies &&
        packageFile.dependencies["react-native"];
      if (!reactNativeVersion) {
        ...
        var linkedJsCodeLocationAssignmentStatement =
          linkTools.linkedJsCodeLocationAssignmentStatement;
        var jsCodeLocations = appDelegateContents.match(
          /(jsCodeLocation = .*)/g
        );
        if (
          !jsCodeLocations ||
          jsCodeLocations.length !== 2 ||
          !~appDelegateContents.indexOf(linkedJsCodeLocationAssignmentStatement)
        ) {
          console.log(`AppDelegate isn't compatible for unlinking`);
        } else {
          if (
            semver.eq(semver.coerce(reactNativeVersion), "0.57.8") ||
            semver.eq(semver.coerce(reactNativeVersion), "0.57.0")
          ) {
            ...
            var linkedCodeLocationPatch = linkTools.getJsCodeLocationPatch(
              defaultJsCodeLocationAssignmentStatement
            );
            appDelegateContents = appDelegateContents.replace(
              linkedCodeLocationPatch,
              defaultJsCodeLocationAssignmentStatement
            );
          }
          fs.writeFileSync(appDelegatePath, appDelegateContents);
