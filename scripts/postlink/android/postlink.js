...
module.exports = () => {
  ...
  if (mainApplicationPath) {
    ...
    if (mainActivityPath) {
      ...
      if (linkTools.isJsBundleOverridden(mainActivityContents)) {
        ...
        var mainActivityClassDeclaration =
          linkTools.mainActivityClassDeclaration;
    ...
    https://github.com/microsoft/react-native-washington-push/blob/master/docs/setup-android.md#plugin-configuration-for-react-native-lower-than-060-android for more details`);
    }
  }
  ...
  if (!fs.existsSync(buildGradlePath)) {
    ...
    https://github.com/microsoft/react-native-washington-push/blob/master/docs/setup-android.md#plugin-installation-android---manual`);
  }
  ...
  if (!stringsResourcesPath) {
    return Promise.reject(
      new Error(
        `Couldn't find strings.xml. You might need to update it manually.`
      )
    );
  } else {
    ...
    if (~stringsResourcesContent.indexOf(deploymentKeyName)) {
      ...
      return inquirer
        ...
        .then(function (answer) {
          ...
          var deploymentKeyString = `\t<string moduleConfig="true" name="${deploymentKeyName}">${
            answer.androidDeploymentKey || "deployment-key-here"
          }</string>`;
