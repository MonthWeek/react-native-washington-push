...
exports.oldBundleUrl =
  '[[NSBundle mainBundle] URLForResource:@"main" withExtension:@"jsbundle"]';
exports.linkedJsCodeLocationAssignmentStatement =
  "jsCodeLocation = [CodePush bundleURL];";

exports.getJsCodeLocationPatch = function (
...
// Fix for https://github.com/microsoft/react-native-washington-push/issues/477
...
exports.getAppDeletePath = function () {
  return (
    findFileByAppName(
      appDelegatePaths,
      packageFile ? packageFile.name : null
    ) || appDelegatePaths[0]
  );
};

exports.getPlistPath = function () {
  var xcodeProjectPaths = glob.sync(
    `**/*.xcodeproj/project.pbxproj`,
    ignoreNodeModulesAndPods
  );
  ...
  try {
    ...
    console.log(
      "Couldn't read info.plist path from xcode project - error: " + e.message
    );
    return getDefaultPlistPath();
  }
  ...
  var plistPathValue =
    getBuildSettingsPropertyMatchingTargetProductName(
      parsedXCodeProj,
      INFO_PLIST_PROJECT_KEY,
      targetProductName,
      RELEASE_BUILD_PROPERTY_NAME
    ) ||
    getBuildSettingsPropertyMatchingTargetProductName(
      parsedXCodeProj,
      INFO_PLIST_PROJECT_KEY,
    ...
    parsedXCodeProj.getBuildProperty(
      INFO_PLIST_PROJECT_KEY,
      RELEASE_BUILD_PROPERTY_NAME
    ) ||
  ...
  //(see https://github.com/microsoft/react-native-washington-push/issues/534#issuecomment-302069326 for details)
  return path.resolve(
    path.dirname(xcodeProjectPath),
    "..",
    plistPathValue.replace(/^"(.*)"$/, "$1")
  );
};
...
function getBuildSettingsPropertyMatchingTargetProductName(
  parsedXCodeProj,
  prop,
  targetProductName,
  build
    ...
    if (!COMMENT_KEY.test(configName)) {
      ...
      if ((build && config.name === build) || build === undefined) {
        if (targetProductName) {
          if (
            config.buildSettings[prop] !== undefined &&
            config.buildSettings[PRODUCT_NAME_PROJECT_KEY] == targetProductName
          ) {
          ...
          if (
            ...
            config.buildSettings[TV_OS_DEPLOYMENT_TARGET_PROPERTY_NAME] ==
              undefined &&
            //exclude test app
