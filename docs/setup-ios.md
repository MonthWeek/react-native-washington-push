...
1. As of v0.27 of React Native, `rnpm link` has already been merged into the React Native CLI. Simply run:
   ...
   ```
   react-native link react-native-washington-push
   ```

   If your app uses a version of React Native that is lower than v0.27, run the following:

   ```
   rnpm link react-native-washington-push
   ```

   _Note: If you don't already have RNPM installed, you can do so by simply running `npm i -g rnpm` and then executing the above command. If you already have RNPM installed, make sure you have v1.9.0+ in order to benefit from this one step install._
...
1. Add the ReactNative and CodePush plugin dependencies to your `Podfile`, pointing at the path where NPM installed it
   ...
   pod 'CodePush', :path => '../node_modules/react-native-washington-push'
   ```

   _NOTE: The above path needs to be relative to your app's `Podfile`, so adjust it as necessary._

   _NOTE: `JWT` library should be >= version 3.0.x_

2. Run `pod install`

_NOTE: The CodePush `.podspec` depends on the `React` pod, and so in order to ensure that it can correctly use the version of React Native that your app is built with, please make sure to define the `React` dependency in your app's `Podfile` as explained [here](https://facebook.github.io/react-native/docs/integration-with-existing-apps.html#podfile)._

#### Plugin Installation (iOS - Manual)

1. Open your app's Xcode project

2. Find the `CodePush.xcodeproj` file within the `node_modules/react-native-washington-push/ios` directory (or `node_modules/react-native-washington-push` for <=`1.7.3-beta` installations) and drag it into the `Libraries` node in Xcode
