...
When you require `react-native-washington-push`, the module object provides the following top-level methods in addition to the root-level [component decorator](#codepush):
...
- [getCurrentPackage](#codepushgetcurrentpackage): Retrieves the metadata about the currently installed update (like description, installation time, size). _NOTE: As of `v1.10.3-beta` of the CodePush module, this method is deprecated in favor of [`getUpdateMetadata`](#codepushgetupdatemetadata)_.
...
- **deploymentKey** _(String)_ - Refer to [`CodePushOptions`](#codepushoptions).
...
- **codePush.SyncStatus.UP_TO_DATE** _(0)_ - The app is fully up-to-date with the configured deployment.
